...
- **Dark column (right):** Price (large) → optional cool-red "↓ $X price reduction · {date}" line → Beds → Baths → Sq.Ft. (each with right-aligned italic label and light gray underline separator)
...
8. **Features** — 2-column bulleted list with light gray checkmarks (only renders if `features.length > 0`)
9. **Location map** — Mapbox map centered on `lat`/`lng` (zoom 14, Standard/night style, single light gray marker pin). Lazy-initialized on scroll into view.
...
- Custom thin light gray scrollbar
...
1. **Color tokens** in the new client's `:root`:
   - `--bronze` (Shana: `#C8C8C8` light gray) → new client's accent color
